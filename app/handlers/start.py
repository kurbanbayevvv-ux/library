...
from database.postgres import kitoblar
...
async def start_handler(message: types.Message):
    ...
    number = 1
    for i in kitoblar():
        response_text += str(number) + ". " +i[0] + "\n"
        number += 1
    fileids = []
    number = 1
    for i in kitoblar():
        fileids.append(InlineKeyboardButton(text=str(number), callback_data=f"book_{i[1]}"))
        number += 1
    keyb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                ...
                InlineKeyboardButton(text=">", callback_data="page_1")
            ]
        ]
    )
    await message.answer(text=response_text, reply_markup=keyb)
